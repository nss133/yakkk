...
# 폐쇄망 검색 UI (반입물 = terms_dist*.db + py 5종: search_app·simmatch·diff_render
#  + clause_split·docx_split(붙여넣기/docx 심사의 지연 import), python3 stdlib만 필요.
#  배포 패키지 조립: _dist/약관검색_v3/ — py 5종+실행 스크립트+사용안내.txt+현행판 DB)
